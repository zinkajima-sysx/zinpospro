...
module.exports = async (req, res) => {
    ...
    const reqRes = await requestJson({
        method: 'GET',
        url: `${base}/rest/v1/subscription_requests?select=id,plan,amount,unique_code,total_amount,status,transfer_name,transfer_date,admin_note,created_at,updated_at&id_toko=eq.${encodeURIComponent(id_toko)}&order=created_at.desc&limit=1`,
        headers: {
            apikey: serviceKey,
            Authorization: `Bearer ${serviceKey}`,
            'Content-Type': 'application/json'
